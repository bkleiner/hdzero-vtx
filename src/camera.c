...
void GetCamCfg(uint8_t USE_EEP_PROFILE)
{
    ...
    #ifdef _DEBUG_CAMERA
    debugf("\r\n    profile:    %x", camProfile);
    debugf("\r\n    brightness: %x", camCfg.brightness);
    debugf("\r\n    sharpness:  %x", camCfg.sharpness);
    debugf("\r\n    saturation: %x", camCfg.saturation);
    debugf("\r\n    contrast:   %x", camCfg.contrast);
    debugf("\r\n    hvFlip:     %x", camCfg.hvFlip);
    debugf("\r\n    nightMode:  %x", camCfg.nightMode);
    debugf("\r\n    wbMode:     %x", camCfg.wbMode);
    #endif
}
