...
void msp_set_vtx_config(uint8_t power, uint8_t save)
{
    ...
    debugf("\r\nF%x,P%x,M:%x",RF_FREQ, power,PIT_MODE);
    #endif
...
void parseMspVtx_V2(uint16_t cmd_u16)
{
    ...
    #ifdef _DEBUG_MODE
    debugf("\r\nparseMspVtx_V2");
    debugf("\r\n    fc_vtx_dev:    %x", msp_rx_buf[0]);
    debugf("\r\n    fc_band_rx:    %x", msp_rx_buf[1]);
    debugf("\r\n    fc_channel_rx: %x", msp_rx_buf[2]);
    debugf("\r\n    fc_pwr_rx:     %x", msp_rx_buf[3]);
    debugf("\r\n    fc_pit_rx :    %x", msp_rx_buf[4]);
    debugf("\r\n    fc_vtx_status: %x", msp_rx_buf[7]);
    debugf("\r\n    fc_lp_rx:      %x", msp_rx_buf[8]);
    debugf("\r\n    fc_bands:      %x", msp_rx_buf[12]);
    debugf("\r\n    fc_channels:   %x", msp_rx_buf[13]);
    debugf("\r\n    fc_powerLevels %x", msp_rx_buf[14]);
    #endif
...
void InitVtxTable()
{
    ...
    if(!powerLock){
        for(i=0;i<9;i++){
            PowerTable[2][i] = power500mW[i];
            PowerTable[3][i] = power1W[i];
        }
        for(i=2;i<5;i++){
            msp_send_header(1);
            crc = 0;
