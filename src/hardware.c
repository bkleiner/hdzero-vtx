...
void PowerAutoSwitch()
{
    ...
    if((!g_IS_ARMED) && (last_ofs == pwr_offset));
        ...
        #ifdef _DEBUG_MODE
        debugf("\r\nPowerAutoSwitch: temp = %x%x, ",(uint8_t)(temp>>8),(uint8_t)(temp&0xff));
...
void PowerAutoSwitch()
{
    ...
    if(last_ofs != pwr_offset){
        #ifdef _DEBUG_MODE
        debugf("\r\nPowerAutoSwitch:Yes %x %x",temp, pwr_offset);
        #endif
        DM6300_SetPower(RF_POWER, RF_FREQ, pwr_offset);
        cur_pwr = RF_POWER;
    }else{
        #ifdef _DEBUG_MODE
        debugf("\r\nPowerAutoSwitch: No %x %x",temp, pwr_offset);
        #endif
    }
                        ...
                        if(pwr_lmt_sec >= 3){
                            if(RF_POWER == 3){
                                ...
                                if(p >= table_power[RF_FREQ][3])
                                    ...
                                    p += 0x4;
                                    debugf("\r\npwr_plus 2dbm,p=%x",(uint8_t)p + pwr_offset);
                                }
